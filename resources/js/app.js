...
async function initMapApp() {
    ...
    attachToolbarHandlers(map, state, watersLayer, stekCluster, rigCluster, () => renderAll());
    ...
    async function hydrate(target) {
        const [watersRes, steksRes, rigsRes, bathyRes] = await Promise.all([
            window.axios.get('/api/waters'),
            window.axios.get('/api/steks'),
            window.axios.get('/api/rigs'),
            window.axios.get('/api/bathy'),
        ]);
    ...
    function showSelection(payload) {
        ...
        const parent = type === 'rig' ? state.steks.find((s) => s.id === item.stek_id) : state.waters.find((w) => w.id === item.water_id);
    ...
    function attachDragHandler(marker, entity, record, appState, rerender) {
        ...
        marker.on('dragend', async (event) => {
            ...
            await window.axios.patch(`/api/${entity}/${record.id}`, payload);
    ...
    function sampleDepth(lat, lng, bathyPoints) {
        ...
        const sorted = bathyPoints
            .map((point) => ({
                point,
                distance: turf.distance(target, turf.point([point.lng, point.lat]), { units: 'kilometers' }),
            }))
    ...
    function attachToolbarHandlers(mapRef, appState, watersLayerRef, stekClusterRef, rigClusterRef, rerender) {
        ...
        toolbarEl?.addEventListener('click', async (event) => {
            ...
            switch (button.dataset.action) {
                ...
                case 'import-bathy':
                    ...
                    break;
                case 'clear-bathy':
                    await window.axios.delete('/api/bathy');
                    appState.bathy = [];
                    rerender();
                    break;
            }
    ...
    async function addWater(mapRef, appState) {
        ...
        const radius = 0.001;
        const geometry = turf.circle([center.lng, center.lat], 0.2, { steps: 32, units: 'kilometers' });
        await window.axios.post('/api/waters', { name, geometry });
        await hydrate(appState);
    }

    async function addStek(mapRef, appState) {
        if (!appState.waters.length) {
            alert('Add a water first.');
            return;
        }
        const name = window.prompt('Stek name?') || 'New swim';
        const center = mapRef.getCenter();
        const waterId = nearestWaterId(center.lat, center.lng, appState.waters);
        await window.axios.post('/api/steks', {
            name,
            lat: center.lat,
            lng: center.lng,
            water_id: waterId,
    ...
    function nearestWaterId(lat, lng, waters) {
        ...
        waters.forEach((water) => {
            ...
            const polygon = turf.feature(water.geometry);
            const distance = turf.pointToLineDistance(point, turf.polygonToLine(polygon), { units: 'kilometers' });
    ...
    function nearestStek(lat, lng, steks) {
        ...
        let best = null;
        steks.forEach((stek) => {
            const distance = turf.distance(point, turf.point([stek.lng, stek.lat]));
            if (!best || distance < best.distance) {
                best = { ...stek, distance };
            }
        });
    ...
    async function importBathy(appState, rerender) {
        ...
        fileInput.onchange = async () => {
            ...
            for (const batch of batches) {
                await window.axios.post('/api/bathy', {
                    points: batch,
                });
            }
            await hydrate(appState);
