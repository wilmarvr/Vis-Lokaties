...
function normalizeDB(){
  ...
  db.steks=(Array.isArray(db.steks)?db.steks:[]).map(function(s){
    ...
    copy.id = copy.id || uid('stek');
    copy.name = copy.name || "";
    copy.note = copy.note || "";
    copy.lat = num(copy.lat);
    copy.lng = num(copy.lng);
    copy.waterId = copy.waterId || null;
    copy.ownerId = (copy.ownerId==null?null:String(copy.ownerId));
    return copy;
  });
  db.rigs=(Array.isArray(db.rigs)?db.rigs:[]).map(function(r){
    var copy=Object.assign({}, r);
    copy.id = copy.id || uid('rig');
    copy.name = copy.name || "";
    copy.note = copy.note || "";
    copy.lat = num(copy.lat);
    copy.lng = num(copy.lng);
    copy.stekId = copy.stekId || null;
    copy.waterId = copy.waterId || null;
    copy.ownerId = (copy.ownerId==null?null:String(copy.ownerId));
    return copy;
  });
  db.waters=(Array.isArray(db.waters)?db.waters:[]).map(function(w){
    var copy=Object.assign({}, w);
    copy.id = copy.id || uid('water');
    copy.name = copy.name || "";
    copy.ownerId = (copy.ownerId==null?null:String(copy.ownerId));
    return copy;
  });
  ...
  if(db.settings.waterColor==null) db.settings.waterColor="#33a1ff";
  if(!('activeUserId' in db.settings)) db.settings.activeUserId=null;
  if(!Array.isArray(db.users)) db.users=[];
  db.users=db.users.map(function(u){
    var copy=Object.assign({}, u);
    copy.id = copy.id || uid('user');
    copy.name = (copy.name==null?"Gebruiker":String(copy.name));
    return copy;
  });
  var seen=new Set();
  db.users=db.users.filter(function(u){ if(seen.has(u.id)) return false; seen.add(u.id); return true; });
  if(db.settings.activeUserId && !db.users.some(function(u){ return u.id===db.settings.activeUserId; })){ db.settings.activeUserId=null; }
}

function currentUserId(){ return (db.settings && db.settings.activeUserId) || null; }
function entryVisibleForCurrent(entry){
  var owner = entry && entry.ownerId;
  var active = currentUserId();
  return !active || owner==null || owner===active;
}
function visibleWaters(){ return (db.waters||[]).filter(entryVisibleForCurrent); }
function visibleSteks(){ return (db.steks||[]).filter(entryVisibleForCurrent); }
function visibleRigs(){ return (db.rigs||[]).filter(entryVisibleForCurrent); }
function assignOwner(entry){
  if(!entry) return entry;
  var owner=currentUserId();
  entry.ownerId = owner||null;
  return entry;
}
function renderUserPanel(){
  ...
  if(!sel) return;
  var active=currentUserId();
  var opts=['<option value="">🌐 Gedeeld (alle data)</option>'];
  (db.users||[]).forEach(function(u){ opts.push('<option value="'+esc(u.id)+'">'+esc(u.name||u.id)+'</option>'); });
  sel.innerHTML=opts.join('');
  sel.value = active || '';
  var btnRen=document.getElementById('btnRenameUser');
  var btnDel=document.getElementById('btnDeleteUser');
  var hasActive=!!active;
  if(btnRen) btnRen.disabled=!hasActive;
...
if(userSelect){
  userSelect.addEventListener('change', function(){
    ...
    db.settings.activeUserId = val||null;
    saveDB();
    renderAll();
    renderUserPanel();
    S(val?('Gefilterd op gebruiker: '+this.options[this.selectedIndex].textContent):'Gedeelde data zichtbaar.');
  });
}
var btnAddUser=document.getElementById('btnAddUser');
if(btnAddUser){
  btnAddUser.addEventListener('click', function(){
    var naam=prompt('Naam nieuwe gebruiker:');
    if(naam==null) return;
    naam=String(naam).trim();
    if(!naam){ alert('Naam mag niet leeg zijn.'); return; }
    var id=uid('user');
    db.users.push({id:id,name:naam});
    db.settings.activeUserId=id;
    saveDB();
    renderUserPanel();
    renderAll();
    S('Gebruiker toegevoegd: '+naam+'.');
  });
}
var btnRenUser=document.getElementById('btnRenameUser');
if(btnRenUser){
  btnRenUser.addEventListener('click', function(){
    var active=currentUserId();
    if(!active){ alert('Selecteer eerst een gebruiker.'); return; }
    var user=(db.users||[]).find(function(u){return u.id===active;});
    if(!user) return;
    var naam=prompt('Nieuwe gebruikersnaam:', user.name||'');
    if(naam==null) return;
    naam=String(naam).trim();
    if(!naam){ alert('Naam mag niet leeg zijn.'); return; }
    user.name=naam;
...
if(btnDelUser){
  btnDelUser.addEventListener('click', function(){
    ...
    var user=(db.users||[]).find(function(u){return u.id===active;});
    if(!user) return;
    if(!confirm('Gebruiker "'+(user.name||user.id)+'" verwijderen? Data wordt gedeeld.')) return;
    ['waters','steks','rigs'].forEach(function(key){
      (db[key]||[]).forEach(function(entry){ if(entry.ownerId===active) entry.ownerId=null; });
    });
    db.users=db.users.filter(function(u){return u.id!==active;});
    if(db.settings.activeUserId===active) db.settings.activeUserId=null;
    saveDB();
    renderUserPanel();
    renderAll();
    S('Gebruiker verwijderd. Data is nu gedeeld.');
  });
}
...
window.renderAll = function(){
  ...
  var waters=visibleWaters();
  var steks=visibleSteks();
  var rigs=visibleRigs();
  ...
  // markers
  steks.forEach(function(s){ var m=makeStekMarker(s); if(useCluster) cluster.addLayer(m); else m.addTo(map); });
  rigs.forEach(function(r){ var m=makeRigMarker(r); if(useCluster) cluster.addLayer(m); else m.addTo(map); });
...
window.drawDistances = function(){
  ...
  steks.forEach(function(s){
    ...
    rigs.filter(function(r){return r.stekId===s.id;}).forEach(function(r){
...
function buildOverview(){
  ...
  waters.forEach(function(w){
    var steksForWater=steks.filter(function(s){return s.waterId===w.id;});
    var rigsCount=0; steksForWater.forEach(function(s){ rigsCount+=rigs.filter(function(r){return r.stekId===s.id;}).length; });
    var tr=document.createElement("tr");
    tr.innerHTML='<td>'+esc(w.name||"(onbenoemd)")+'</td><td>'+w.id+'</td><td>'+steksForWater.length+'</td><td>'+rigsCount+'</td>'+
  ...
  ts.querySelectorAll(".btnReWaterStek").forEach(function(b){
    b.onclick = async function(ev){
      ...
      var arr=visibleWaters().map(function(w){
        var f=(w.geojson && w.geojson.features && w.geojson.features[0])?w.geojson.features[0]:null;
  ...
  trc.querySelectorAll(".btnReStekRig").forEach(function(b){
    b.onclick = async function(ev){
      ...
      var r = (db.rigs||[]).find(function(x){return x.id===id;});
      if(!r) return;
      var arr=visibleSteks().map(function(s){
...
function saveWaterFeature(feat,name){
  ...
  db.waters.push(assignOwner({id:id,name:name,geojson:fc})); saveDB(); renderAll(); S("Water aangemaakt: "+name);
}
...
function pointsInViewport(maxTake){
  ...
  visibleSteks().forEach(function(s){ if(b.contains([s.lat,s.lng])) pts.push([s.lng,s.lat]); });
  visibleRigs().forEach(function(r){ if(b.contains([r.lat,r.lng])) pts.push([r.lng,r.lat]); });
...
document.getElementById('btnLocalLoad').addEventListener('click', function(){
  try{ var raw=localStorage.getItem(DB_KEY); if(raw){ db=JSON.parse(raw); normalizeDB(); renderUserPanel(); renderAll(); S('Data geladen uit browser.'); } else { S('Geen data gevonden in browser.'); } }catch(e){ S('Laden mislukt.'); }
});
document.getElementById('btnLocalReset').addEventListener('click', function(){
  ...
  db={waters:[],steks:[],rigs:[],bathy:{points:[],datasets:[]},settings:{waterColor:"#33a1ff",activeUserId:null},users:[]};
  normalizeDB();
  renderUserPanel();
...
document.getElementById('btnSaveHtmlWithData').addEventListener('click', function(){
  try{
    ...
    if(!snapEl){
      ...
      snapEl.id = 'lv_db_snapshot';
      document.body.appendChild(snapEl);
      created = true;
    }
    var prev = snapEl.textContent;
  ...
  }catch(e){ S('Kon snapshot niet insluiten.'); }
});
...
function scoreSpot(lat, lon, sekitar, rigList){
  ...
  var nearPenalty = 0;
  (rigList||[]).forEach(function(r){ var dd=distM({lat:lat,lon:lon},{lat:r.lat,lon:r.lng}); if(dd<12) nearPenalty -= (12-dd)*0.2; });
...
document.getElementById('btnAutoRigs').addEventListener('click', function(){
  var b=map.getBounds();
  var added=0;
  var rigList=visibleRigs().slice();
  visibleSteks().forEach(function(s){
    ...
    radii.forEach(function(rm){
      ...
      bearings.forEach(function(br){
        ...
        var sc = scoreSpot(lat,lon,s,rigList);
    ...
    picked.forEach(function(p,k){
      var owner = (s.ownerId!=null)?s.ownerId:currentUserId();
      var rig={id:uid('rig'),name:'Rig '+(k+1),lat:p.lat,lng:p.lng,stekId:s.id,waterId:s.waterId||nearestWaterIdForLatLng(p.lat,p.lng)||null,ownerId:owner||null};
      db.rigs.push(rig);
      rigList.push(rig);
      added++;
    });
