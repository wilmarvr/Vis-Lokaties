...
- The front-end performs `fetch` requests against `api/db.php`, which now splits the JSON payload across normalized tables (`waters`, `steks`, `spots`, `bathy_points`, `bathy_datasets`, `settings`) before serving the combined document back to the browser. Every bathymetry save also records dataset metadata (name, counts, depth range, bounding box, timestamp) so the `bathy_datasets` table always mirrors the UI list. Deeper imports call `saveDBImmediate()` as soon as the parser finishes, so the MySQL tables are updated even if the user never clicks “Save”.
